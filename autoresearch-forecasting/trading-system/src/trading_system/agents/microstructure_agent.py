...
This agent fetches live data from Upstox, runs the 7-layer scoring math,
...
def run_microstructure(
    ...
    ``expected_volume_now`` is the time-of-day pro-rated expected volume for
    this instrument (used to compute RVOL). Operators provide a sensible
    default per instrument; small inaccuracy is tolerated since RVOL is a
    coarse filter.
    """

    chain = client.option_chain(instrument_key)
    of = compute_features(chain)
    spot = of.spot

    quote = client.full_quote([instrument_key])
    main_quote = next(iter(quote.get("data", {}).values()), {})
    bids, asks = _depth_levels(main_quote)

    intraday = client.intraday_candles(instrument_key, "5minute")
    ...
    if related_keys:
        try:
            ohlc = client.ohlc(related_keys, interval="1d")
            for k, v in ohlc.get("data", {}).items():
                ltp = float(v.get("last_price", 0))
                prev_close = float(v.get("ohlc", {}).get("close", ltp))
                if prev_close > 0:
                    related_pct[k] = (ltp - prev_close) / prev_close * 100
        except Exception:
            related_pct = {}
