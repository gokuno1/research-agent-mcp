"""
Daily Nifty 50 next-day direction prediction using saved GBT model.

Usage:
    python predict.py                                  # predict next trading day
    python predict.py --model saved_models/model_dir/  # use a specific model directory
    python predict.py --days 5                         # show last 5 days + next prediction
...
DEFAULT_MODEL_DIR = os.path.join(
    SCRIPT_DIR, "saved_models", "model_0p6609_20260408_113217"
)
...
# ---------------------------------------------------------------------------
# Model loading — GBT (GradientBoostingClassifier) saved as pickle
# ---------------------------------------------------------------------------


# ---------------------------------------------------------------------------
# Feature engineering — copied from train.py to keep predict.py standalone
# ---------------------------------------------------------------------------
...
def load_model(model_dir):
    """Load GBT model from pickle and metadata from checkpoint.pt."""
    pkl_path = os.path.join(model_dir, "gbt_model.pkl")
    ckpt_path = os.path.join(model_dir, "checkpoint.pt")

    if not os.path.exists(pkl_path):
        print(f"ERROR: GBT model not found: {pkl_path}")
        sys.exit(1)
...
def predict_next_day(clf, meta, merged_df):
    """
    Build features from merged data, clean NaN/Inf (NO normalization),
    take the last row as input, and predict with the GBT model.
    """
    features_df = build_features(merged_df)

    if len(features_df) < 1:
        print("ERROR: No valid feature rows after engineering")
        sys.exit(1)

    feat_np = features_df.values
    expected_cols = meta.get("input_dim", feat_np.shape[1])
    ...
    feat_clean = np.nan_to_num(feat_np, nan=0.0, posinf=0.0, neginf=0.0)

    last_row = feat_clean[-1:, :]
    pred_class = clf.predict(last_row)[0]
    ...
    return {
        ...
        "nifty50_last_close": round(float(merged_df["nifty50_close"].iloc[-1]), 2),
...
def main():
    parser = argparse.ArgumentParser(
        description="Nifty 50 next-day direction prediction")
    parser.add_argument("--model", default=DEFAULT_MODEL_DIR,
                        help="Path to saved model directory (containing gbt_model.pkl)")
    ...
    print("=" * 60)
    print("  Nifty 50 Next-Day Direction Predictor (GBT)")
    print("=" * 60)
    print()

    print(f"Model: {os.path.basename(model_dir)}")
    clf, meta = load_model(model_dir)
    print(f"  Type: {meta.get('model_type', 'GBT')}")
    print(f"  Accuracy (walk-forward): {meta.get('accuracy', 'N/A'):.4f}")
    print(f"  Trained: {meta.get('timestamp', 'N/A')}")
    print(f"  Features: {meta.get('input_dim', '?')}")
    print()
    ...
    result = predict_next_day(clf, meta, merged)
    print()
